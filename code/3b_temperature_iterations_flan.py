...
hdbscan_model = HDBSCAN(min_cluster_size=100,min_samples=1, metric='euclidean',cluster_selection_epsilon=0.05, 
                        cluster_selection_method='leaf', prediction_data=True,core_dist_n_jobs=4,memory='tmp/')
...
OUTPUT_DIR = '../results/topic_model/temperature_runs_flan'
MAX_RETRIES = 3
...
def build_flan_representation(temperature: float) -> TextGeneration:
    pipeline_kwargs = {
        "max_new_tokens": 50,
    }
    if temperature > 0:
...
def build_flan_representation_bundle(temperature: float) -> dict:
    temp_suffix = _format_temperature_suffix(temperature)
    key = f"flan_lnp_t{temp_suffix}"
    return {key: build_flan_representation(temperature)}
...
for temperature in TEMPERATURES:
    for iteration in range(1, ITERATIONS_PER_TEMPERATURE + 1):
        ...
        # Enforce variation in generation per attempt
        set_seed(iteration + int(temperature * 100))

        for attempt in range(1, MAX_RETRIES + 1):
            try:
                temp_topic_model = copy.deepcopy(base_topic_model)
                representation_models = {"Main": main_representation}
                representation_models.update(build_flan_representation_bundle(temperature))
                ...
                # Output the updated predictions, overwriting existing ones!
                ...
                print(
                    f"[temperature={temperature} iteration={iteration}] FLAN labelling failed (attempt {attempt}/{MAX_RETRIES}): {err}. "
